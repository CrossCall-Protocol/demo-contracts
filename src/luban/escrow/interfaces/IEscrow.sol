...
interface IEscrow {
  ...
  function depositAndLock(address asset_, uint256 amount_) external payable;
  ...
  error PaymasterPaymentFailed(address receiver, address asset, uint256 amount);
  ...
  error testerror(bytes);

  event PrintUserOp(PackedUserOperation userOp);
}
